	...
	if(!empty($_POST)){
		if(isset($_POST["username"]) &&isset($_POST["fullname"]) &&isset($_POST["email"]) &&isset($_POST["password"]) &&isset($_POST["confirm_password"])){
			if($_POST["username"]!=""&& $_POST["fullname"]!=""&&$_POST["email"]!=""&&$_POST["password"]!=""&&$_POST["password"]==$_POST["confirm_password"]){
				...
				$sql = "insert into user(username,fullname,email,password,created_at, activation_key, validated) value (\"$_POST[username]\",\"$_POST[fullname]\",\"$_POST[email]\",\"$_POST[password]\",NOW(),\"$random_key\",\"$validated\")";
				$query = $con->query($sql);
				if($query!=null){
